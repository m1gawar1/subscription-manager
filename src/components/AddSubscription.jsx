    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }} className="no-scrollbar">
        {filtered.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', paddingBottom: '8px' }}>
            {filtered.map(preset => (
              <button
                key={preset.id}
                type="button"
              ...
              >
                <div style={{ width: '60px', height: '60px', borderRadius: '16px', backgroundColor: 'var(--input-bg)', border: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', position: 'relative' }}>
                  <CategoryIcon id={preset.categoryId} size={22} color={CATEGORIES.find(c => c.id === preset.categoryId)?.color || 'var(--text-muted)'} />
                  {preset.domain && (
                    <img
